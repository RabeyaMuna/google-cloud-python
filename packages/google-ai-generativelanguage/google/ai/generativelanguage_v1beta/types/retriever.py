...
class Corpus(proto.Message):
    r"""A ``Corpus`` is a collection of ``Document`` s. A project can create up to 5 corpora.
    ...
    Attributes:
        name (str):
            ...
            name is empty on create, a unique name will be derived from ``display_name``
...
class Document(proto.Message):
    r"""A ``Document`` is a collection of ``Chunk`` s. A ``Corpus`` can have a maximum of 10,000 ``Document`` s.
    ...
    Attributes:
        name (str):
            ...
            start or end with a dash. If the name is empty on create, a unique name will
            be derived from ``display_name`` along with a 12 character random suffix.
...
class StringList(proto.Message):
    r"""User provided string values assigned to a single metadata key.
...
class CustomMetadata(proto.Message):
    ...
    This message has oneof_ fields (mutually exclusive fields). For each oneof, at most
    one member field can be set at the same time. Setting any member of the oneof
    automatically clears all other members.
...
class MetadataFilter(proto.Message):
    r"""User provided filter to limit retrieval based on ``Chunk`` or ``Document`` level metadata values. Example (genre = drama OR genre = action): key = "document.custom_metadata.genre" conditions = [{string_value = "drama", operation = EQUAL}, {string_value = "action", operation = EQUAL}]
...
class Condition(proto.Message):
    ...
    class Operator(proto.Enum):
        r"""Defines the valid operators that can be applied to a key-value pair.
        ...
        Values:
            ...
            INCLUDES (7):
                ...
                given key has a ``string_list_value``.
...
class Chunk(proto.Message):
    r"""A ``Chunk`` is a subpart of a ``Document`` that is treated as an independent unit for the purposes of vector representation and storage. A ``Corpus`` can have a maximum of 1 million ``Chunk`` s.
